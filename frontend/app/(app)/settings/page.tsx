...
import { Building2, Save, KeyRound, User, Globe, Check } from 'lucide-react';
...
import { formatHostelId, formatMasterId, formatStudentId } from '@/lib/utils';
          ...
          <div className="flex items-center justify-between border-b border-[#CBD5E1] pb-3">
            ...
            {user && (
              <span className="text-[11px] font-mono font-bold bg-[#EFF6FF] text-[#2563EB] px-2.5 py-1 rounded-lg border border-[#BFDBFE]">
                {user.role === 'STUDENT' ? `Student ID: ${formatStudentId(user)}` : `Master ID: ${formatMasterId(user)}`}
              </span>
            )}
          </div>
